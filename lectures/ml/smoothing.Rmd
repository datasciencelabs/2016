...
p <- ggplot(dat2, aes(X, Y)) +
  ...
  geom_vline(aes(xintercept = center, frame = center), lty = 2) +
  geom_point(shape = 1, data = dat, alpha = .5,cex=5) +
  geom_line(aes(x=X, y = .fitted, frame = X, cumulative = TRUE), data = loess_fit, color = "red") +
  ggtitle("x0 = ")

gg_animate(p, "loess.gif", ani.width = 1050, ani.height = 700, interval=0.15)
...
With k=51 we obtain what appears to be a decent estimate of the true $f$.
...
```{r, echo=FALSE}
f_hat <- predict(knn_fit, newdata = test_set, k=51)[,2]
...
low_prob <-  which(abs(f_hat-0.5)<0.02)[1:5] 
