...
**Your answer here**:
Because we have no prior idea who will win, the prior mean $\mu$ 
should be 0. And again, we do not know anything about the data, so
 $\tau$ should be big. 
...
1 - pnorm(0, tabEst$theta_hat, 
          tabEst$sigma_hat / sqrt(tabEst$tot_poll))
```
...
tab %>% 
    filter(start_date>"2008-01-01" & N == 1000) %>%
    summarize(total_polls = n(), 
              poll_size = unique(N),
              theta_hat = mean(diff), 
              sigma_hat_theoretical = 2*sqrt(mean(Obama)*(1-mean(Obama))) / sqrt(unique(N)))
```
...
tab %>% 
    ...
    summarize(total_polls = n(), 
              poll_size = unique(N), 
              theta_hat = mean(diff), 
              sigma_hat_theor = 2*sqrt(mean(Obama)*(1-mean(Obama))) / sqrt(unique(N)), 
...
qplot(rnorm(N,mu_post,sd_post), geom = "histogram", bins = 20, 
      xlab = "difference between Obama and McCain")
```
