...
$$ \varepsilon \sim \mbox{Normal}\left(0, \frac{2\sqrt{\theta (1-\theta)}}{\sqrt{n}}\right) $$

where $n$ is the poll size from a single poll. This means statistical
theory tells us the expected value of $\varepsilon$ is 0 and the 
standard deviation $\sigma$ should be given by
$\frac{2\sqrt{\theta (1-\theta)}}{\sqrt{n}}$. 

Remember, if $\theta$ represented the percentage for just one 
...
$$ \frac{\sqrt{\theta (1-\theta)}}{\sqrt{n}} $$

But if we are estimating the difference in percentages between two 
candidates, then the estimate is approximated by 
$\hat{\theta} - (1-\hat{\theta}) = 2\hat{\theta} - 1$. 
This implies that the standard error is $2 \sqrt{ \theta (1 - \theta)} / \sqrt{n}$.

Because each $Y_i$ a separate poll then the standard deviation of 
$\varepsilon$ should be about $2 \sqrt{ \theta (1 - \theta) }/ \sqrt{n}$:
...
what is the chance that Obama will beat 
McCain in the 2008 presidential election? 
...
to the election. Note that we also want to 
assume $w$ is an auto-correlated process. 
...
for $\theta$ that down-weight bad pollsters. 
