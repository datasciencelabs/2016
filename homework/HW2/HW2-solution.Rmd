...
the risk of mortgage-backed securities (MBS) and collateralized debt obligations (CDO), whose price dependens on homeowners making their monthly payments, were grossly underestimated. A combination of factors resulted in many more defaults than were expected. This resulted in a crash of the prices of these securities. As a consequence, banks lost so much money that they needed bailouts to avoid default.
...
Answer:
Expectation is $\mu = -1 \times (1-p) + 1\times p$ which is -1/19. The casino makes, on average, about 5 cents on each bet.
...
Answer: Normal approxmation tells us the average profit $S/N$ has expected value $\mu=$`expectation` and standard deviation $\sigma/\sqrt{N}$ with $\sigma=$ `(interest_rate2*loan + loss_per_foreclosure)*sqrt(p2*(1-p2))`. So we compute

$$\mbox{Pr}(S/N > 0) = \mbox{Pr}(\sqrt{N}*(S/N - \mu)/\sigma > -\sqrt{N}*\mu/\sigma)
