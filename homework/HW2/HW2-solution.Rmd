...
Answer:
Expectation is $\mu = -1 \times (1-p) + 1\times p$ which is -1/19 or about 5 cents. 
...
Note that the Monte Carlo simulation gave a slightly higher probability that 0.001. What is a possible reason for this? 
...
Answer:
Note that expected profits are the same:
```{r}
mean(profit)
mean(profit2)
```
